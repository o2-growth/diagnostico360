...
import { LayoutDashboard, Settings, Building2, History, PanelLeftClose, PanelLeftOpen, Download, Home, LogOut, Menu } from "lucide-react";
...
const SidePanel = ({ onTabChange, onMenuToggle }: SidePanelProps) => {
  ...
  return (
    <>
      {/* Mobile hamburger toggle - visible only when sidebar is closed on mobile */}
      {isMobile && !isOpen && (
        <Button
          variant="ghost"
          size="icon"
          className="fixed top-4 left-4 z-50 hover:bg-white/10 transition-colors duration-200"
          onClick={toggleMenu}
        >
