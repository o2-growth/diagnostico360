...
const HistoryDetail = () => {
  ...
  const overallScore = snapshot.overall_score;
  const classification = getClassification(overallScore);
  const textColor = theme === 'dark' ? '#e5e7eb' : '#1f2937';

  const completedDate = format(new Date(snapshot.completed_at), "dd 'de' MMMM 'de' yyyy", { locale: ptBR });
  const completedDateTime = format(new Date(snapshot.completed_at), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR });
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className={`transition-all duration-300 ${isMenuExpanded ? 'pl-64' : 'pl-16'} print:pl-0`}>
        <div className="p-8 max-w-6xl mx-auto">
          {/* Action buttons */}
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              {questionGroups.map((group) => {
                const score = deptScores[group.id] ?? 0;
                ...
                <div key={group.id} className="dashboard-card mb-6 overflow-hidden">
                  ...
                  {gate === 'nao' ? (
                    <p className="text-sm" style={{ color: '#EF4444' }}>
                      Área marcada como inexistente neste diagnóstico.
                    </p>
                  ) : (
                    <>
                      {/* Questions Table */}
          ...
          <footer className="text-center text-dashboard-muted text-sm py-8 border-t border-white/10">
            ...
            <p className="mt-1">Relatório de {completedDate}</p>
          </footer>
        </div>
      </div>
    </div>
